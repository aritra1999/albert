...
<div class="flex justify-between items-center border-b boder-slate-300 py-4">
    <input class="rounded-lg border-none bg-slate-100 text-slate-800 text-sm font-medium" type="date" on:change={updateEvents} bind:value={localeDateString}>
    <div class="flex items-center space-x-4">
		<div class="flex items-center rounded hover:bg-slate-100 py-2 px-2 sm:px-4">
            <span class="hidden sm:block">Events</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-slate-700 font-semibold ml-0 sm:ml-2">
